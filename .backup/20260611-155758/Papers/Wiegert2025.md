...
Modélisation normative profonde (espace latent à 256 dimensions) sur sMRI de deux grandes cohortes (NAKO ~29 000 ; UK Biobank ~25 000). Carte de déviation individualisée multivariée : la magnitude de déviation croît avec la sévérité symptomatique, les patrons directionnels séparent dépression/anxiété et usage d'alcool et généralisent entre cohortes. Classification individuelle quantitative (régression logistique sur les 256 déviations) : AUC jusqu'à ~74 % (déviations seules, PHQ-9 sévère) et ~80 % combinée aux PRS ; gains moyens d'AUC de 5,6 % (ANX), 7,2 % (MDD), 3,7 % (alcool). Biomarqueur computationnel/personnalisé sans région universelle.
