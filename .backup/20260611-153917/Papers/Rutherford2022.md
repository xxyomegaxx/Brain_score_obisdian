...
Article-ressource construisant des modèles normatifs de référence (épaisseur corticale / volume sous-cortical ; N=58 836, 82 sites). Il génère des cartes de déviation individualisées et montre que les z-scores normatifs renforcent les effets cas-témoins par rapport aux données brutes, mais aucun biomarqueur discriminatif validé n'est rapporté : les auteurs diffèrent explicitement la signification clinique. Preuve discriminative préliminaire, non quantifiée par une métrique de classification.

> « For most diagnostic groups, the z-statistics derived from the normative deviations also provided stronger case-control effects than the raw data. »
